...
async def command(cmd: Command, authorization: str | None = Header(None)):
    auth(authorization)
    if cmd.stopId not in (1,2): raise HTTPException(400, "stopId must be 1 or 2")
    ws = vehicle_ws.get(cmd.vehicleId)
    if not ws: raise HTTPException(409, "Vehicle offline")
    rid = cmd.requestId or str(uuid.uuid4())
    await ws.send_json({"type":"command","cmd":"GO_TO_STOP","stopId":cmd.stopId,"requestId":rid})
